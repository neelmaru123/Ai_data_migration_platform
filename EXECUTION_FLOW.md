...
- **[NEW]**: [`apps/api/app/modules/sources/sources_routes.py`](file:///c:/Neel/AI%20DATA%20MIGRATION%20PLATFORM/apps/api/app/modules/sources/sources_routes.py) - FastAPI endpoints for testing connections, introspecting schemas, and uploading CSV/Excel files.
- **[NEW]**: [`apps/api/tests/unit/test_sources_connectors.py`](file:///c:/Neel/AI%20DATA%20MIGRATION%20PLATFORM/apps/api/tests/unit/test_sources_connectors.py) - Unit test suite for factories, connectors, and file loaders.
- **[MODIFIED]**: [`apps/api/app/core/config.py`](file:///c:/Neel/AI%20DATA%20MIGRATION%20PLATFORM/apps/api/app/core/config.py#L28-L32) - Updated Pydantic settings to dynamically discover `.env` files across root and API module levels.
