...
| :--- | :--- |
| **`main.py`** | Agent daemon process. Runs startup multi-threaded TCP socket connectivity tests against configured database URLs (`SRC_*_URL`, `DEST_*_URL`), runs a dedicated background daemon heartbeat thread (`start_heartbeat_thread`) firing every 20s during ETL jobs, triggers schema introspection, and polls for pending execution jobs (`GET /api/v1/agents/tasks`). |
...
| **`execution_engine.py`** (`ExecutionOrchestrator`) | The local ETL execution engine: <br>1. **`DDLExecutor`**: Executes pre/post-migration DDL scripts on the target database.<br>2. **`SourceConnectorFactory`**: Reads source data in 50,000-row streaming chunks with SQL and Mongo `_id`-based keyset pagination.<br>3. **`ASTTransformer`**: Applies in-memory Polars transformations (type casting, 4 PK strategies: `keep_original`, `autoincrement_offset`, `prefix_id`, `uuid_v4_rekey`, and residual `extra_attributes` JSON capture).<br>4. **`TableMerger`**: Merges multi-source tables via bounded-memory DuckDB local staging tables with streaming deduplication.<br>5. **`CheckpointManager`**: Persists row offsets locally per table and per source for crash resilience.<br>6. **`TargetWriterFactory`**: Performs high-performance bulk insertions with `rowcount` conflict verification and PyMongo `BulkWriteError` handling.<br>7. **`ProgressReporter`**: Sends real-time progress updates (including `skipped_rows`) to backend via HTTPS. |
